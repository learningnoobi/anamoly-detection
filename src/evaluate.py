...
def measure_response_time(model, loader, device, n_warmup=3):
    ...
    return {
        ...
        "std_latency_ms": float(per_batch_ms.std()),
...
def feature_selection_mi(X, y, save_path="./checkpoints/feature_importance_mi.png"):
    ...
    print(
        f"  Computing MI on {len(X):,} flows x {len(X.columns)} features …",
        flush=True,
        end=" ",
    )
    ...
    df = (
        pd.DataFrame({"feature": Xm.columns, "MI": mi})
...
def classifier_fairness(y_true, y_pred, save_path="./checkpoints/fairness.png"):
    """
    ...
    c_tpr = [
        "#2ecc71" if t > 0.95 else "#f39c12" if t > 0.8 else "#e74c3c" for t in tpr
    ]
    ...
    a2.axhline(
        fpr.max(), color="red", ls=":", lw=1.5, label=f"max FPR = {fpr.max():.6f}"
    )
    a2.set(
        xticks=x,
        xticklabels=ATTACK_TYPES,
...
def _train_one_fold(
    ...
    cm = confusion_matrix(
        np.concatenate(labels), np.concatenate(preds), labels=range(NUM_CLASSES)
    )
    return cm, best_acc


def kfold_confusion_matrices(
    X,
    y,
    n_folds=5,
    device="cpu",
    ...
    **kw,
    ...
    _draw(
        fig.add_subplot(gs[n_fold_rows, :]),
        avg_cm,
        f"Average ({n_folds}-Fold)  —  {np.mean(accs):.4f} ± {np.std(accs):.4f}",
    )

    plt.suptitle("K-Fold CV Confusion Matrices  (row-normalised)", fontsize=12, y=1.01)
    Path(save_path).parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    print(f"  Saved → {save_path}")
    plt.close()
